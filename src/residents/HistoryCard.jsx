        ...
        <div className="bg-white rounded-2xl md:rounded-3xl shadow-sm border border-gray-100 overflow-hidden mt-8">
            ...
            <div className="p-4 md:p-6 border-b border-gray-50 flex items-center justify-between">
                <h3 className="text-lg md:text-xl font-bold text-gray-800 flex items-center gap-2">
                    <Receipt className="text-indigo-500" size={22} /> Billing History
                </h3>
                <span className="hidden sm:block bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full text-xs font-bold uppercase">
                    {history.length} Records
                </span>
            </div>

            <div className="w-full">
                {/* --- MOBILE VIEW: Stacked Cards (Visible on mobile only) --- */}
                <div className="block md:hidden divide-y divide-gray-50">
                    {history.map((item) => {
                        const status = getStatusDetails(item.status);
                        const txId = item.paymentHistory?.length > 0 
                            ? item.paymentHistory[item.paymentHistory.length - 1].transactionId 
                            : 'No Ref';
                        ...
                        return (
                            <div key={item._id} className="p-4 space-y-3">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Billing Month</p>
                                ...
                                <div className="pt-1 flex items-center gap-2 text-xs text-gray-400">
                                    <span className="font-mono">{txId !== 'No Ref' ? txId : 'No Transaction ID'}</span>
                                    {txId !== 'No Ref' && <ExternalLink size={12} />}
                                </div>
                            </div>
                        );
                    })}
                            ...
                            {history.map((item) => {
                                ...
                                return (
                                    <tr key={item._id} className="hover:bg-gray-50/50 transition-colors group">
                                        <td className="px-6 py-4 font-bold text-gray-700 whitespace-nowrap">{item.billingMonth}</td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2 max-w-[150px]">
                                                <p className="text-xs text-gray-400 font-mono truncate">
                                                    {txId !== 'No Ref' ? txId : 'No Ref'}
                                                </p>
                                                {txId !== 'No Ref' && <ExternalLink size={12} className="text-gray-300 group-hover:text-indigo-400 transition-colors" />}
                                            </div>
                                        </td>
                ...
                {history.length === 0 && (
                    <div className="py-16 md:py-24 text-center px-6">
                        <div className="bg-gray-50 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                            <Receipt className="text-gray-200" size={32} />
                        </div>
                        <p className="text-gray-400 font-medium italic">No payment history found.</p>
                    </div>
                )}
            </div>
        </div>
